...
# ==========================================
st.sidebar.title("🎛️ Operational Controls")
# FEATURE 1: WHAT-IF HIRING SLIDER
st.sidebar.subheader("Strategic Planning")
extra_hires = st.sidebar.slider("Simulate New Hires per Market", 0, 10, 0)
avg_capacity = 25 # Assume 25 sessions per week for new hires
...
if page == "📊 Executive Dashboard":
    ...
    # Apply the "What-If" logic to our supply
    query = f"""
        SELECT 
            market,
            active_clinicians,
    ...
    col1.metric("Avg Market Utilization", f"{health_df['simulated_utilization'].mean():.1f}%", 
                delta=f"{health_df['simulated_utilization'].mean() - 92:.1f}%", delta_color="inverse")
    col2.metric("Simulated New Supply", f"+{extra_hires * 10} Clinicians", f"{extra_hires * 10 * avg_capacity} sessions")
    col3.metric("Critical Markets", len(health_df[health_df['simulated_utilization'] > 100]))

    st.subheader("Regional Utilization (Simulated)")
    st.bar_chart(data=health_df, x="market", y="simulated_utilization")
    
    # FEATURE 4: THE WAITLIST MAP (SIMPLE LAT/LON MOCK)
    st.subheader("📍 Market Demand Heat Map")
    # We'll map our markets to rough US coordinates for the visual
...
GROUP BY market
    ...
    # FEATURE 2: ONBOARDING VELOCITY (TIME-TO-FILL)
    st.subheader("⏱️ Onboarding Velocity Audit")
    velocity_query = """
        SELECT 
            market,
    ...
    st.caption("Red bars indicate markets where onboarding takes >30 days (Operational Drag).")

    # FEATURE 3: DATA QUALITY SCORECARD
    st.subheader("🛡️ Data Integrity Audit")
    raw_patients = pd.read_csv("data/raw_patients.csv")
    
    total_rows = len(raw_patients)
    bad_dates = len(raw_patients[raw_patients['churn_date'] < raw_patients['intake_date']])
    missing_market = raw_patients['market'].isna().sum()
    
    c1, c2, c3 = st.columns(3)
    c1.metric("Data Health Score", f"{((total_rows - (bad_dates + missing_market))/total_rows)*100:.1f}%")
    c2.metric("Rejected: Date Logic", bad_dates)
    c3.metric("Rejected: Missing Routing", missing_market)
    
    st.dataframe(raw_patients[(raw_patients['market'].isna()) | (raw_patients['churn_date'] < raw_patients['intake_date'])].head(10))
